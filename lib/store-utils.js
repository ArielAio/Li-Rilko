...
export function buildWhatsAppMessage(cartItems, settings) {
  ...
  const unavailableItems = cartItems
    ...
    .map((item) => `• ${item.name}`)
    .join("\n");
  const unavailableWarning = unavailableItems
    ? `\n\n⚠️ Itens indisponíveis no momento:\n${unavailableItems}\nPor favor, confirme a disponibilidade antes de fechar o pedido.`
    : "";

  return `${intro}\n\n${lines.join("\n")}\n\nTotal à vista: ${formatCurrency(totalCash)}\nTotal a prazo: ${formatCurrency(totalInstallment)}${unavailableWarning}`;
}
