...
function buildWhatsAppLinkFromText(text, settings, preferredPhone = "") {
  const siteSettings = resolveSiteSettings(settings);
  const normalizedPhone = normalizePhone(preferredPhone) || normalizePhone(siteSettings.whatsappPhone);
...
export function buildWhatsAppLink(cartItems, settings, preferredPhone = "") {
  const text = buildWhatsAppMessage(cartItems, settings);
  return buildWhatsAppLinkFromText(text, settings, preferredPhone);
}

export function buildFloatingWhatsAppLink(settings, preferredPhone = "") {
  const siteSettings = resolveSiteSettings(settings);
  return buildWhatsAppLinkFromText(siteSettings.whatsappFloatingMessage, siteSettings, preferredPhone);
}
